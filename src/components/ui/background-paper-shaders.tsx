...
const fragmentShader = `
  ...
  void main() {
    ...
    color = mix(color, vec3(1.0), pow(abs(noise), 2.0) * intensity * 0.05);
...
export function EnergyRing({
  ...
  useFrame((state) => {
    if (mesh.current) {
      ...
      // In ThreeJS the material is accessed via mesh.current.material, but TypeScript needs casting
...
export function BackgroundPaperShaders() {
  return (
    <div className="absolute inset-0 z-0 w-full h-full pointer-events-none bg-black">
      ...
      >
        <ShaderPlane position={[0, 0, 0]} color1="#111111" color2="#030303" />
      </Canvas>
    </div>
  );
}
